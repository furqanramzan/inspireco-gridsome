...
async function checkForImages(item, assetUrl) {
  if (assetUrl) {
    for (const itemKey in item) {
      const itemContent = item[itemKey];
      if (itemContent && itemContent.type && imageTypes.includes(itemContent.type)) {
        item[itemKey].gridsome_image = await download(`${assetUrl}/${itemContent.id}?download`, itemContent.filename_disk);
      } else if (itemContent && itemKey !== 'owner' && typeof itemContent === 'object' && Object.keys(itemContent).length > 0) {
        item[itemKey] = await checkForImages(itemContent, assetUrl);
      }
    }
  }
...
class DirectusSource {
  ...
  async fetchContent(store) {
    ...
    const { apiUrl, assetUrl, project, staticToken, email, password, collections, maxRetries, reconnectTimeout } = this.options
    ...
    const client = new Directus('http://127.0.0.1:8055/');
    ...
    let connect = async () => {
      return new Promise(async (resolve, reject) => {
        try {
          await client.auth.login(Object.assign({ email, password, persist: false }, directusOptions));

          resolve(await client.items("directus_collections"));
        } catch (e) {
    ...
    for (const collection of collections) {
      ...
      try {
        ...
        let data = await client.items(directusPathName).readMany(params);
        data = data.data;
        ...
        for (let item of data) {
          ...
          if (assetUrl && params.downloadImages) {
            item = await checkForImages(item, assetUrl);
          }

          if (assetUrl && params.downloadFiles) {
            item = await checkForDownloads(item, assetUrl);
          }
